...
model_data_08    =  'tot_prec_original_JJAS_diurnal_remap_30.nc'

model_data_06    =  'tot_prec_original_JJAS_diurnal_remap_30.nc'

model_data_05    =  'tot_prec_original_JJAS_diurnal_remap_30.nc'

model_data_obs_imerg    =  'IMERG_2007_2016_hourly_JJAS_diurnal_30.nc'
...
###
    ...
    print(f"local_peak_time_08_map dtype: {local_peak_time_08_map.dtype}")
    print(f"local_peak_time_08_map shape: {local_peak_time_08_map.shape}")
    print("\nFirst few values:")
    print(local_peak_time_08_map.isel(lat=0, lon=0).values)
    print(local_peak_time_08_map.isel(lat=1, lon=1).values)

    # Also check the original 'time' variable
    print(f"\ntime variable dtype: {time.dtype}")
    print(f"time values: {time.values}")

    diff_local_peak_time_08_map = local_peak_time_08_map - local_peak_time_obs_imerg_map
    diff_local_peak_time_06_map = local_peak_time_06_map - local_peak_time_obs_imerg_map
    diff_local_peak_time_05_map = local_peak_time_05_map - local_peak_time_obs_imerg_map

    diff_local_peak_time_08_map = (diff_local_peak_time_08_map + 12) % 24 - 12
    diff_local_peak_time_06_map = (diff_local_peak_time_06_map + 12) % 24 - 12
    diff_local_peak_time_05_map = (diff_local_peak_time_05_map + 12) % 24 - 12




    print("DEBUG - Checking data types:")
    print(f"diff_local_peak_time_08_map dtype: {diff_local_peak_time_08_map.dtype}")
    print(f"diff_local_peak_time_08_map shape: {diff_local_peak_time_08_map.shape}")
    print("\nFirst few values:")
    print(diff_local_peak_time_08_map.isel(lat=0, lon=0).values)
    print(diff_local_peak_time_08_map.isel(lat=1, lon=1).values)
...
######
 ...
 #   '#bd0026', '#a80023', '#800026', '#4d0015'   # Red to Deep Red
    ...
    tick_doys = np.arange(-6, 6.1, 0.5)  # 10-day intervals within the March-July range
    norm = BoundaryNorm(tick_doys, cmap.N)
...
#  Preparing for the figures & Spceifying the Projections
 ...
 #   local_peak_time_obs_imerg_map.plot.pcolormesh(ax=ax1, transform=ccrs.PlateCarree(), cmap=custom_cmap,  add_colorbar=False, add_labels=False)
    ...
    diff_local_peak_time_08_map.plot.pcolormesh(ax=ax1, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm,  add_colorbar=False, add_labels=False)   
 
    ax2 = fig.add_subplot(3,3,i+1, projection=proj)
    diff_local_peak_time_06_map.plot.pcolormesh(ax=ax2, transform=ccrs.PlateCarree(), cmap=cmap,  norm=norm,  add_colorbar=False, add_labels=False)    

    ax3 = fig.add_subplot(3,3,i+2, projection=proj) 
    diff_local_peak_time_05_map.plot.pcolormesh(ax=ax3, transform=ccrs.PlateCarree(), cmap=cmap,  norm=norm, add_colorbar=False, add_labels=False)


    all_axes = []
    if i == 7:
           all_axes.extend([ax1, ax2, ax3])
    ...
    ax1.set_title('ICON (10) - IMERG (Reference)',fontweight='bold', fontsize=10)
    ...
    ax2.set_title('ICON (40KM) - IMERG',fontweight='bold', fontsize=10)
...
#    ax2.set_title('Average Monsoon Onset Date (Calendar Format - 10 Day Intervals)')
    ...
    ax3.set_title('ICON (80KM) - IMERG',fontweight='bold', fontsize=10)
...
fig.savefig("diff_Fig7_Diurnal_Spatial.pdf", bbox_inches='tight', pad_inches=0.1,dpi=50)
