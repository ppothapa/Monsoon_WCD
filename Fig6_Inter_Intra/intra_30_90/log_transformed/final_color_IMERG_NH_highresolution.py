...
data_base_dir = '/capstor/store/cscs/userlab/cwp03/ppothapa/Paper_1_10km_Monsoon/Fig6_Inter_Intra/intra_30_90/log_transformed/data'

model_data_08    =  'evolve_var_08_summer_intraseasonal_var.nc'

model_data_06    =  'evolve_var_06_summer_intraseasonal_var.nc'

model_data_05    =  'evolve_var_05_summer_intraseasonal_var.nc'

model_data_obs_imerg    =  'evolve_var_imerg_summer_intraseasonal_var.nc'
...
# Read the Variable from Here.  
    ...
    var_obs_imerg = ds_obs_imerg["__xarray_dataarray_variable__"]
...
## Select for CORE
    ...
    var_obs_imerg_core = var_obs_imerg.sel(lat=slice(lat_min_core,lat_max_core),lon=slice(lon_min_core,lon_max_core))
...
####
    ...
    var_08_core = var_08_core * mask_imerg_core
    var_06_core = var_06_core * mask_imerg_core
    var_05_core = var_05_core * mask_imerg_core
    var_obs_imerg_core = var_obs_imerg_core * mask_imerg_core


######

    var_08 = var_08/var_obs_imerg
    var_06 = var_06/var_obs_imerg
    var_05 = var_05/var_obs_imerg

    var_08 = var_08.where(var_08 != 0)
    var_06 = var_06.where(var_06 != 0)
    var_05 = var_05.where(var_05 != 0)



#######
  
    var_08_core = var_08_core/var_obs_imerg_core
    var_06_core = var_06_core/var_obs_imerg_core
    var_05_core = var_05_core/var_obs_imerg_core

    var_08_core = var_08_core.where(var_08_core != 0)
    var_06_core = var_06_core.where(var_06_core != 0)
    var_05_core = var_05_core.where(var_05_core != 0)
...
#    tick_doys = np.arange(0, 3.1, 0.03)  # 10-day intervals within the March-July range
#    tick_doys_bar = np.arange(0, 3.1, 0.6)
#    norm = BoundaryNorm(tick_doys, cmap.N)

# New Color Scheme

    cmap = mpl.colormaps['RdBu_r']  # Perfect: Red=positive bias, Blue=negative bias, White=zero
    tick_doys = np.arange(0, 3.1, 0.03)  # 10-day intervals within the March-July range
    tick_doys_bar = np.arange(0, 3.1, 0.3)
    #norm = TwoSlopeNorm(vmin=0, vcenter=1.0, vmax=3.0) 
    #norm = BoundaryNorm(tick_doys, cmap.N)


    tick_doys = np.concatenate([
    np.linspace(0, 0.999, 128),    # Stop just before 1.0
    np.linspace(1.0, 3, 128)     # Start at 1.0
    ])
    norm = BoundaryNorm(tick_doys, cmap.N)

# OR for better visual with white at 1:
#    norm = mpl.colors.TwoSlopeNorm(vmin=0, vcenter=1, vmax=3.0)
...
#     plot = diff_sum_win_06.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['g'],linewidths=2.5)
  ...
  # 
    diff_data_clean = [d[d != 0][~np.isnan(d[d != 0])] for d in diff_data]
   ...
   # Create box plot
    ...
    ax4.set_ylabel('Variance Ratio (30-60 day)', fontweight='bold', fontsize=10)
    ax4.set_title('Variance Ratio', fontweight='bold', fontsize=12)
    ax4.grid(True, alpha=0.3)
    ax4.axhline(y=1, color='grey', linestyle='-', linewidth=1.5, alpha=0.7, zorder=0)
...
# Access the colorbar and set ticks/labels
           ...
           cbar.set_ticks(tick_doys_bar)  # Set ticks at 10-day intervals (DOY)
#           cbar.set_ticklabels(tick_labels)  # Set the tick labels as calendar dates (e.g., 01 Mar, 11 Mar, ...)
           cbar.set_label('Ratio of Mean Variance for precipitation', fontsize=12, fontweight='bold')
...
# Title and axis labels
    ...
    ax1.set_title('ICON (10KM)/IMERG (Ratio)',fontweight='bold', fontsize=10)
    ...
    ax2.set_title('ICON (40KM)/IMERG (Ratio)',fontweight='bold', fontsize=10)
...
#    ax2.set_title('Average Monsoon Onset Date (Calendar Format - 10 Day Intervals)')
    ax3.set_title('ICON (80KM)/IMERG (Ratio)',fontweight='bold', fontsize=10)
    ...
    if i == 1:
            ...
            ax4.set_title('(d) Distribution',fontweight='bold', fontsize=14)
    ...
    if i == 5:
            ...
            ax4.set_title('(h) Distribution',fontweight='bold', fontsize=14)
    ...
    if i == 9:
            ...
            ax4.set_title('(l) Distribution',fontweight='bold', fontsize=14)



#plt.tight_layout()
#plt.savefig(plot_name)
fig.savefig("Fig7_Intra_30_90_logp.pdf", bbox_inches='tight', pad_inches=0.1)
