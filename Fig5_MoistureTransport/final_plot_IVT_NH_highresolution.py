...
#  Preparing for the figures & Spceifying the Projections
    ...
    if i == 1:
             ...
             arrow_scale=2000
             skip_value=10
             bounds = np.linspace(0,6,20)
             bounds = np.arange(0,6.1,0.2)
             norm = mcolors.BoundaryNorm(boundaries=bounds, ncolors=256)
             norm = mcolors.BoundaryNorm(boundaries=bounds, ncolors=len(bounds)-1)
    # Use fewer colors to avoid the error
             cmap = mpl.colormaps['Spectral_r']
             colors = ['white'] + [cmap(j) for j in np.linspace(0, 1, len(bounds)-2)]
             cmap = mcolors.LinearSegmentedColormap.from_list('Spectral_r_white', colors, N=len(bounds)-1)
             ...
             skip_value=8
             arrow_scale=1500
             bounds = np.linspace(0,0.8,20)
             bounds = np.arange(0,0.81,0.04)
             norm = mcolors.BoundaryNorm(boundaries=bounds, ncolors=256)
             norm = mcolors.BoundaryNorm(boundaries=bounds, ncolors=len(bounds)-1)
    # Use fewer colors to avoid the error
             cmap = mpl.colormaps['Spectral_r']
    ...
    var_08_mag.plot(ax=ax2, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
    # 3. Add quiver — optionally slice to avoid overcrowding
    skip = skip_value  # adjust to thin arrows
    ax2.quiver(
    var_08_qu['lon'][::skip], 
    var_08_qu['lat'][::skip],
    var_08_qu.values[::skip, ::skip], 
    var_08_qv.values[::skip, ::skip],
...
#     plot = diff_sum_win_08.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['r'],linewidths=2.5) 
    ...
    var_06_mag.plot(ax=ax3, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
#     plot = diff_sum_win_06.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['g'],linewidths=2.5)
    skip = skip_value  # adjust to thin arrows
    ax3.quiver(
    var_06_qu['lon'][::skip],
    var_06_qu['lat'][::skip],
    var_06_qu.values[::skip, ::skip],
    var_06_qv.values[::skip, ::skip],
    transform=ccrs.PlateCarree(),
    scale=arrow_scale,         # adjust scaling of arrows
    width=0.005,       # adjust thickness
    headwidth=3
#    scale_units='xy'
    ...
    plot_obj = var_05_mag.plot(ax=ax4, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, cbar_kwargs={'pad': 0.02, 'label': 'Integrated Vertical Transport (Kg m-1 s-1)'})
...
#     plot = diff_sum_win_05.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['b'],linewidths=2.5)
    ...
    var_05_qu['lon'][::skip],
    var_05_qu['lat'][::skip],
    var_05_qu.values[::skip, ::skip],
    var_05_qv.values[::skip, ::skip],
    transform=ccrs.PlateCarree(),
    scale=arrow_scale,         # adjust scaling of arrows
    width=0.005,       # adjust thickness
    headwidth=3
#    scale_units='xy'
    ...
    var_obs_era5_mag.plot(ax=ax1, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
    skip = skip_value  # adjust to thin arrows
    ax1.quiver(
    var_obs_era5_qu['lon'][::skip],
    var_obs_era5_qu['lat'][::skip],
    var_obs_era5_qu.values[::skip, ::skip],
    var_obs_era5_qv.values[::skip, ::skip],
    transform=ccrs.PlateCarree(),
    scale=arrow_scale,         # adjust scaling of arrows
    width=0.005,       # adjust thickness
    headwidth=3
...
# Title and axis labels
    ...
    ax1.set_title('ERA5 (Reference)',fontweight='bold', fontsize=14)
...
#    _ = fig.subplots_adjust(left=0.2, right=0.8, hspace=0, wspace=0, top=0.8, bottom=0.25)
    ...
    if i == 5:
            ax1.set_title('(e) ERA5 (Reference)',fontweight='bold', fontsize=14)
    ...
    if i == 9:
            ax1.set_title('(i) ERA5 (Reference)',fontweight='bold', fontsize=14)
...
fig.savefig("IVT_Transport.pdf", bbox_inches='tight', pad_inches=0.1)
