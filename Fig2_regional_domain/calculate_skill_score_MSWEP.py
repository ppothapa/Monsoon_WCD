...
#    scores_08 = {};scores_06 = {};scores_05 = {}    
    ...
    scores_08[x] = compute_skill_scores(mask_08, mask_mswep)
    scores_06[x] = compute_skill_scores(mask_06, mask_mswep)
    scores_05[x] = compute_skill_scores(mask_05, mask_mswep)
...
#  Preparing for the figures & Spceifying the Projections
    ...
    if i == 6 or i == 2 or i ==4:
	     			mask_mswep.plot(ax=ax,transform=ccrs.PlateCarree(), cmap='Greys',vmin=0,vmax=1,cbar_kwargs={'label': 'Mask', 'extend': 'both',  'shrink': .8, 'orientation': 'vertical'},add_labels=False)

    else:
        mask_mswep.plot(ax=ax,transform=ccrs.PlateCarree(), cmap='Greys',vmin=0,vmax=1,add_colorbar=False,add_labels=False)
