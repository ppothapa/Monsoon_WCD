...
#     plot = diff_sum_win_05.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['b'],linewidths=2.5)
    ...
    if i == 6 or i == 2 or i ==4:
	     			diff_sum_win_obs_imerg_nan.plot(ax=ax,transform=ccrs.PlateCarree(), cmap='Spectral_r',vmin=0,vmax=16,cbar_kwargs={'label': 'Total Precipitation (mm/day)', 'extend': 'both',  'shrink': .8, 'orientation': 'vertical'},add_labels=False)
...
#    _ = fig.subplots_adjust(left=0.2, right=0.8, hspace=0, wspace=0, top=0.8, bottom=0.25)
    ...
    if i == 6:
        ...
        ax.legend(legend_handles, labels, loc='lower left', fontsize=12)
