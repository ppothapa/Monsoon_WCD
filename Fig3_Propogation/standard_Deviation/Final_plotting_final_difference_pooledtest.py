...
# Select the Seasons to Caluculate the Monsoon Onset & Do Pentad Centered over a day!      
    ...
    var_obs_imerg_summer = var_obs_imerg.sel(time=var_obs_imerg.time.dt.month.isin([3,4,5,6,7]))
    var_obs_imerg_summer = var_obs_imerg_summer.rolling(time=5, center=True).mean()
...
#   Multiply with the Mask. 
    var_08_summer            = var_08_summer * mask_imerg 
    var_06_summer            = var_06_summer * mask_imerg
    var_05_summer            = var_05_summer * mask_imerg
    var_obs_imerg_summer     = var_obs_imerg_summer  * mask_imerg
    ...
    'var_obs_imerg_summer': var_obs_imerg_summer,
    'var_obs_cpc_summer': var_obs_cpc_summer,
    'var_obs_era5_summer': var_obs_era5_summer,
    'var_obs_mswep_summer': var_obs_mswep_summer,
     } 

    model_names=['var_08_summer', 'var_06_summer', 'var_05_summer', 'var_obs_imerg_summer' , 'var_obs_cpc_summer', 'var_obs_era5_summer',  'var_obs_mswep_summer']
...
# Average across years
        ...
        sd_onset_day   = onset_doy.std(dim='year')
        key = f"{model_name}_onset"
        onset_data[key] = mean_onset_doy
        sd_onset_data[key] = sd_onset_day
...
# v vonset_data[key] = mean_onset_doy
    ...
    var_obs_imerg_onset = onset_data["var_obs_imerg_summer_onset"]  
    ...
    var_08_onset_sd        = sd_onset_data["var_08_summer_onset"]
    var_06_onset_sd        = sd_onset_data["var_06_summer_onset"]
    var_05_onset_sd        = sd_onset_data["var_05_summer_onset"]
    var_obs_imerg_onset_sd = sd_onset_data["var_obs_imerg_summer_onset"]
    var_obs_cpc_onset_sd   = sd_onset_data["var_obs_cpc_summer_onset"]
    var_obs_era5_onset_sd  = sd_onset_data["var_obs_era5_summer_onset"]
    var_obs_mswep_onset_sd = sd_onset_data["var_obs_mswep_summer_onset"]
    ...
    var_obs_imerg_onset_clean = smart_clean_onset(var_obs_imerg_onset)
  
    
    var_08_onset_sd_clean = smart_clean_onset(var_08_onset_sd)
    var_06_onset_sd_clean = smart_clean_onset(var_06_onset_sd)
    var_05_onset_sd_clean = smart_clean_onset(var_05_onset_sd)
    var_obs_imerg_onset_sd_clean = smart_clean_onset(var_obs_imerg_onset_sd)
 
#    # LATER: Filter out points where either had no onset
#    def filter_no_onset_differences(diff_da, onset1, onset2, no_onset_value=200):
#        """Remove differences where either dataset had no onset"""
#        valid_onset_mask = (onset1 < no_onset_value) & (onset2 < no_onset_value)
#        return diff_da.where(valid_onset_mask)
#
#       # Apply filter for box plot
#        var_08_onset_diff_filtered = filter_no_onset_differences(
#                                     var_08_onset_difference, 
#                                      var_08_onset_clean, 
#                                      var_obs_imerg_onset_clean
#                                       )

 
# Difference in the domain
    var_08_onset_difference        = var_08_onset_clean - var_obs_imerg_onset_clean 
    var_06_onset_difference        = var_06_onset_clean - var_obs_imerg_onset_clean
    var_05_onset_difference        = var_05_onset_clean - var_obs_imerg_onset_clean

    var_08_onset_difference_clean = var_08_onset_difference.where(var_08_onset_difference != 0)
    var_06_onset_difference_clean = var_06_onset_difference.where(var_06_onset_difference != 0)
    var_05_onset_difference_clean = var_05_onset_difference.where(var_05_onset_difference != 0)

    var_08_onset_difference_clean_core = var_08_onset_difference_clean.sel(lat=slice(lat_min_core,lat_max_core),lon=slice(lon_min_core,lon_max_core))

    var_06_onset_difference_clean_core = var_06_onset_difference_clean.sel(lat=slice(lat_min_core,lat_max_core),lon=slice(lon_min_core,lon_max_core))
   
    var_05_onset_difference_clean_core = var_05_onset_difference_clean.sel(lat=slice(lat_min_core,lat_max_core),lon=slice(lon_min_core,lon_max_core))


   # var_08_significant = abs(var_08_onset_difference_clean) > var_08_onset_sd_clean
   # var_06_significant = abs(var_06_onset_difference_clean) > var_06_onset_sd_clean
   # var_05_significant = abs(var_05_onset_difference_clean) > var_05_onset_sd_clean



    # t-critical for 95% confidence with 18 degrees of freedom (10+10-2)
    t_critical = 2.101
#    t_critical = 1.734

# Calculate combined standard error
    se_08 = np.sqrt(var_08_onset_sd_clean**2/10 + var_obs_imerg_onset_sd_clean**2/10)
    se_06 = np.sqrt(var_06_onset_sd_clean**2/10 + var_obs_imerg_onset_sd_clean**2/10)
    se_05 = np.sqrt(var_05_onset_sd_clean**2/10 + var_obs_imerg_onset_sd_clean**2/10)

# Significant if difference > t_critical * se
    var_08_significant = abs(var_08_onset_difference_clean) > (t_critical * se_08)
    var_06_significant = abs(var_06_onset_difference_clean) > (t_critical * se_06)
    var_05_significant = abs(var_05_onset_difference_clean) > (t_critical * se_05)
    ...
    tick_doys = np.arange(-60, 61, 1)  # 10-day intervals within the March-July range
...
#    var_obs_imerg_onset.plot.contourf(ax=ax1, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)

    ax1 = fig.add_subplot(3,4,i, projection=proj)
     # Plot countours for 10KM, 40KM and 80KM
    var_08_onset_difference_clean.plot(ax=ax1, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
    ...
    ax1.contourf(var_08_significant.lon, var_08_significant.lat, 
             var_08_significant, levels=[0.5, 1.5], 
...
#     plot = diff_sum_win_08.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['r'],linewidths=2.5) 
    ...
    var_06_onset_difference_clean.plot(ax=ax2, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
...
#     plot = diff_sum_win_06.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['g'],linewidths=2.5)
    ...
    ax2.contourf(var_06_significant.lon, var_06_significant.lat,
             var_06_significant, levels=[0.5, 1.5],
             transform=ccrs.PlateCarree(),
             hatches=['.....', None],
             colors='none',
             alpha=0)
    ...
    var_05_onset_difference_clean.plot(ax=ax3, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm,  add_colorbar=False, add_labels=False)
    ax3.set_extent([lon_min, lon_max, lat_min, lat_max], crs=ccrs.PlateCarree())  # FORCE EXTENT

    ax3.contourf(var_05_significant.lon, var_05_significant.lat,
             var_05_significant, levels=[0.5, 1.5],
             transform=ccrs.PlateCarree(),
             hatches=['.....', None],
             colors='none',
             alpha=0)
...
#         var_obs_mswep_onset.plot(ax=ax, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, cbar_kwargs={'pad': 0.02})
   ...
   # Prepare data for box plot
    diff_data = [
    var_08_onset_difference_clean_core.values.flatten(),
    var_06_onset_difference_clean_core.values.flatten(), 
    var_05_onset_difference_clean_core.values.flatten()
     ]
   ...
   # Remove NaN values
    ...
    ax4.set_ylabel('Onset Difference (days)', fontweight='bold', fontsize=10)
    ax4.set_title('Distribution of Onset Differences', fontweight='bold', fontsize=12)
...
#           cbar.set_ticklabels(tick_labels)  # Set the tick labels as calendar dates (e.g., 01 Mar, 11 Mar, ...)
           cbar.set_label('Average Monsoon Onset Bias',fontsize=12, fontweight='bold')
...
# Title and axis labels
    ...
    if i == 1:
            ax1.set_title('(a) ICON(10KM) - IMERG',fontweight='bold', fontsize=14)
            ax2.set_title('(b) ICON(40KM) - IMERG',fontweight='bold', fontsize=14)
            ax3.set_title('(c) ICON(80KM) - IMERG',fontweight='bold', fontsize=14)
            ax4.set_title('(d) Distribution of Biases',fontweight='bold', fontsize=14)

    if i == 5:
            ax1.set_title('(e) ICON(10KM) - IMERG',fontweight='bold', fontsize=14)
            ax2.set_title('(f) ICON(40KM) - IMERG',fontweight='bold', fontsize=14)
            ax3.set_title('(g) ICON(80KM) - IMERG',fontweight='bold', fontsize=14)
            ax4.set_title('(h) Distribution of Biases',fontweight='bold', fontsize=14)

    if i == 9:
            ax1.set_title('(i) ICON(10KM) - IMERG',fontweight='bold', fontsize=14)
            ax2.set_title('(j) ICON(40KM) - IMERG',fontweight='bold', fontsize=14)
            ax3.set_title('(k) ICON(80KM) - IMERG',fontweight='bold', fontsize=14)
...
fig.savefig("Fig5_Monsoon_Onset_Bias.pdf", bbox_inches='tight', pad_inches=0.1)
