...
# Mask Data Sets
data_base_mask = '/capstor/store/cscs/userlab/cwp03/ppothapa/Paper_1_10km_Monsoon/Fig1_domains/masks'
...
mask_cpc='cpc_mask_2mm_55_na.nc'
...
#    mask_imerg    = mask_imerg.transpose()
...
#  Select Lat & Lon forcusing on the Masks. 
...
# v vonset_data[key] = mean_onset_doy
    ...
    var_08_onset = var_08_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    var_06_onset = var_06_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    var_05_onset = var_05_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    var_obs_imerg_onset = var_obs_imerg_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    var_obs_cpc_onset = var_obs_cpc_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    var_obs_era5_onset = var_obs_era5_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    var_obs_mswep_onset = var_obs_mswep_onset.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))

    # Ensure exact same grid before subtraction
    def regrid_to_match(da_source, da_target):
        """Regrid source to match target grid exactly"""
        return da_source.interp(lat=da_target.lat, lon=da_target.lon, method='linear')

    # Before subtraction, handle NaN values
    def safe_subtract(da_model, da_obs, max_reasonable_diff=60):
        """Safely subtract onset dates with NaN handling"""
    # Create a mask of valid points in both datasets
        valid_mask = ~np.isnan(da_model) & ~np.isnan(da_obs)
    
    # Initialize result with NaNs
        result = xr.full_like(da_model, np.nan)
    
    # Perform subtraction only where both are valid
        result = da_model - da_obs
    
    # Clip unreasonable differences (more than 60 days)
        result = result.where(np.abs(result) <= max_reasonable_diff)
    
        return result

# Regrid model data to match observation grid
    var_08_onset_regrid = regrid_to_match(var_08_onset, var_obs_imerg_onset)
    var_06_onset_regrid = regrid_to_match(var_06_onset, var_obs_imerg_onset)
    var_05_onset_regrid = regrid_to_match(var_05_onset, var_obs_imerg_onset)

# Now subtract
    var_08_onset_bias = safe_subtract(var_08_onset_regrid, var_obs_imerg_onset)
    var_06_onset_bias = safe_subtract(var_06_onset_regrid, var_obs_imerg_onset)
    var_05_onset_bias = safe_subtract(var_05_onset_regrid, var_obs_imerg_onset)

    cmap = mpl.colormaps['Spectral_r']
    # Define the boundaries for the colorbar ticks (DOY from March 1st to July 31st)
    tick_doys = np.arange(-40, 40, 4)  # 10-day intervals within the March-July range
...
#  Preparing for the figures & Spceifying the Projections
    ...
    print("=== FINAL GUARANTEED SOLUTION: MANUAL PLOTTING ===")

    def manual_plot_guaranteed(ax, da, cmap, norm, lat_min, lat_max, lon_min, lon_max):
        """Manual plotting that guarantees domain control"""
    
    # Extract the actual data and coordinates as numpy arrays
        if da.dims == ('y', 'x'):  # If using nuclear option
           lats = da.lat.values
           lons = da.lon.values
           data = da.values
        else:  # Normal case
    ...
    # FORCE the domain - this is critical
        ax.set_extent([lon_min, lon_max, lat_min, lat_max], crs=ccrs.PlateCarree())
    
        return contour

    # Use manual plotting for all subplots
    ax1 = fig.add_subplot(3,4,i, projection=proj)
    manual_plot_guaranteed(ax1, var_08_onset_bias, cmap, norm, lat_min, lat_max, lon_min, lon_max)
    ax1.coastlines()

    ax2 = fig.add_subplot(3,4,i+1, projection=proj)
    manual_plot_guaranteed(ax2, var_06_onset_bias, cmap, norm, lat_min, lat_max, lon_min, lon_max)
    ax2.coastlines()

    ax3 = fig.add_subplot(3,4,i+2, projection=proj)
    manual_plot_guaranteed(ax3, var_05_onset_bias, cmap, norm, lat_min, lat_max, lon_min, lon_max)
    ax3.coastlines()    

    ax4 = fig.add_subplot(3,4,i+3, projection=proj)
    manual_plot_guaranteed(ax4, var_obs_imerg_onset, cmap, norm, lat_min, lat_max, lon_min, lon_max)
    ax4.coastlines()    
...
#           cbar.set_ticklabels(tick_labels)  # Set the tick labels as calendar dates (e.g., 01 Mar, 11 Mar, ...)
           cbar.set_label('Average Monsoon Onset Date')
...
fig.savefig("Fig3_Monsoon_domains_difference_1up.pdf", bbox_inches='tight', pad_inches=0.1)
