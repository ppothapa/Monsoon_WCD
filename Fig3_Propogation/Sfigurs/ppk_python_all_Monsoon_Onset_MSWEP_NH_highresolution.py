...
# v vonset_data[key] = mean_onset_doy
    ...
    tick_doys = np.arange(60, 211, 20)  # 10-day intervals within the March-July range
...
#  Preparing for the figures & Spceifying the Projections
    ...
    var_obs_mswep_onset.plot.contourf(ax=ax1, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)

    ax2 = fig.add_subplot(3,4,i+1, projection=proj)
     # Plot countours for 10KM, 40KM and 80KM
    var_08_onset.plot.contourf(ax=ax2, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
#     plot = diff_sum_win_08.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['r'],linewidths=2.5) 
    ...
    var_06_onset.plot.contourf(ax=ax3, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm, add_colorbar=False, add_labels=False)
#     plot = diff_sum_win_06.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['g'],linewidths=2.5)
    ...
    var_05_onset.plot.contourf(ax=ax4, transform=ccrs.PlateCarree(), cmap=cmap, norm=norm,  add_colorbar=False, add_labels=False)
...
# Title and axis labels
    ...
    ax1.set_title('MSWEP (Reference)',fontweight='bold', fontsize=10)
...
fig.savefig("Fig3_Monsoon_domains_MSWEP.pdf", bbox_inches='tight', pad_inches=0.1)
