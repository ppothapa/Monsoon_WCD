...
for x in reg:
    ...
    #South Asia
          ...
          lon_min=-18
          lon_max=16
          plot_name='Guinea Coast'
          plot_name = '(e) WAfriM (Guiena Coast)'
          k = 5
          ...
          lon_max=-90
          plot_name='North America'
          plot_name = '(f) NAmerM (Central America)'
          k = 6
...
#logbins = np.logspace(np.log10(bins[0]),np.log10(bins[-1]),len(bins))
 ...
 #    ax.set_xlabel("Precipitation (mm/day)", fontweight='bold', fontsize=12)
     ...
     if k == 4:
        ax.set_ylabel("Probablity Density", fontweight='bold', fontsize=12)
     ax.set_title(plot_name, fontweight='bold', fontsize=12)
...
#                       projection=ccrs.PlateCarree())
     ...
     if k ==2: 
        inset_left = (k-1)/3 + 0.06  # Manual positioning based on subplot index
        inset_bottom = 0.2  # Fixed bottom position
        inset_width  = 0.2   # Fixed width
        inset_height = 0.2  # Fixed height
...
fig.savefig("Fig8_PDF_plot_sub.pdf", bbox_inches='tight', pad_inches=0.1)
