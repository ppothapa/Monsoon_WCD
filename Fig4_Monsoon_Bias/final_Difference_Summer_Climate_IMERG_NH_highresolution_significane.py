...
sig_model_08  = 'hatch_08_summer.nc'

sig_model_06  = 'hatch_06_summer.nc'

sig_model_05  = 'hatch_05_summer.nc'
...
sig_model_08_dir   = os.path.join(data_base_dir,sig_model_08)
sig_model_06_dir   = os.path.join(data_base_dir,sig_model_06)
sig_model_05_dir   = os.path.join(data_base_dir,sig_model_05) 
...
# Load hatching file
ds_hatch_08 = xr.open_dataset(sig_model_08_dir)
ds_hatch_06 = xr.open_dataset(sig_model_06_dir)
ds_hatch_05 = xr.open_dataset(sig_model_05_dir)
...
# Read the Variable from Here.  
    ...
    hatch_var_08 = ds_hatch_08["__xarray_dataarray_variable__"]
    hatch_var_06 = ds_hatch_06["__xarray_dataarray_variable__"]
    hatch_var_05 = ds_hatch_05["__xarray_dataarray_variable__"]
...
# Select Lat & Lon focussing on the tropics 
    ...
    hatch_08 = hatch_var_08.sel(lat=slice(lat_min,lat_max),lon=slice(lon_min,lon_max))
    hatch_06 = hatch_var_06.sel(lat=slice(lat_min,lat_max),lon=slice(lon_min,lon_max))
    hatch_05 = hatch_var_05.sel(lat=slice(lat_min,lat_max),lon=slice(lon_min,lon_max))
...
##  
    hatch_08 = hatch_08 * mask_imerg
    hatch_06 = hatch_06 * mask_imerg
    hatch_05 = hatch_05 * mask_imerg
...
#     plot = diff_sum_win_08.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['r'],linewidths=2.5) 
    ...
    ax1.contourf(hatch_08.lon, hatch_08.lat, hatch_08,
             levels=[0.5, 1.5], transform=ccrs.PlateCarree(),
             hatches=['...', None], colors='none', alpha=0)
...
#     plot = diff_sum_win_06.plot.contour(ax=ax1,levels=[-2], transform=ccrs.PlateCarree(), colors=['g'],linewidths=2.5)
    ...
    ax2.contourf(hatch_06.lon, hatch_06.lat, hatch_06,
             levels=[0.5, 1.5], transform=ccrs.PlateCarree(),
             hatches=['...', None], colors='none', alpha=0)
    ...
    ax3.contourf(hatch_05.lon, hatch_05.lat, hatch_05,
             levels=[0.5, 1.5], transform=ccrs.PlateCarree(),
             hatches=['...', None], colors='none', alpha=0)
...
# Title and axis labels
    ...
    ax1.set_title('ICON (10KM) -IMERG',fontweight='bold', fontsize=14)
    ...
    ax2.set_title('ICON (40KM) - IMERG',fontweight='bold', fontsize=14)
...
#    ax2.set_title('Average Monsoon Onset Date (Calendar Format - 10 Day Intervals)')
    ax3.set_title('ICON (80KM) - IMERG',fontweight='bold', fontsize=14)
...
fig.savefig("Fig6_Bias_Precipitation.pdf", bbox_inches='tight', pad_inches=0.1)
